...
import { Language, DBTopic, DBVocabulary, DBExercise, AIGeneratedSentence } from '../types';
import { SpeakerWaveIcon, CheckCircleIcon, XCircleIcon, LightbulbIcon } from '../components/icons';
...
const LessonPage: React.FC = () => {
    ...
    const [aiSentences, setAiSentences] = useState<AIGeneratedSentence[]>([]);
    ...
    const handleNextVocab = () => {
        if (lessonData && currentVocabIndex < lessonData.vocab.length - 1) {
            ...
            setStep(LessonStep.AI_PRACTICE);
        }
    };
    
    const fetchAIPractice = async () => {
        if(!lessonData) return;
        setIsLoadingAI(true);
        // Map DBVocabulary to service expected format
        const serviceVocab = lessonData.vocab.map(v => ({
            id: v.id || 0,
            indonesian: v.indonesian,
            english: user?.learning_language === Language.ENGLISH ? v.target_word : '...',
            mandarin: user?.learning_language === Language.MANDARIN ? v.target_word : '...',
            pinyin: v.pinyin || ''
        }));
        
        const sentences = await generatePracticeSentences(serviceVocab, user?.learning_language === Language.ENGLISH ? 'English' : 'Mandarin');
                         ...
                         <div className="space-y-4 w-full max-w-2xl">
                           {aiSentences.map((s, i) => (
                              <div key={i} className="bg-white p-4 rounded-lg shadow-sm border">
                                <p className="font-semibold text-gray-800">{s.indonesian}</p>
                                <p className="text-green-700">{user?.learning_language === Language.ENGLISH ? s.english : `${s.mandarin} (${s.pinyin})`}</p>
                              </div>
                           ))}
                         </div>
                         {aiSentences.length === 0 && (
                            <button onClick={fetchAIPractice} disabled={isLoadingAI} className="mt-6 bg-blue-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:bg-blue-700 disabled:bg-blue-300 transition-colors">
                                {isLoadingAI ? 'Membuat kalimat...' : 'Buat Kalimat Latihan'}
                            </button>
                         )}
                         {aiSentences.length > 0 && (
                             <button onClick={handleStartQuiz} className="mt-8 w-full max-w-md bg-green-600 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:bg-green-700 transition-colors">
                                Mulai Kuis!
                            </button>
                         )}
                         {/* Bypass if AI fails or user skips */}
                         <button onClick={handleStartQuiz} className="mt-4 text-gray-500 underline text-sm">Lewati Latihan AI</button>
                    </>
                )
